...
function LoginLogsPanel({ token }: { token: string }) {
  ...
  const loadLogs = async () => {
    ...
    try {
      ...
      if (data.recentFailures) setRecentFailures(data.recentFailures);
    } catch (err) {
  ...
  useEffect(() => {
    loadLogs();
  }, [token, emailFilter, successFilter]);

  return (
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-white">登录日志审计</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-white/70 text-sm">总登录次数</span>
          </div>
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-white/70 text-sm">成功登录</span>
          </div>
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-white/70 text-sm">失败登录</span>
          </div>
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-white/70 text-sm">成功率</span>
          </div>
          <p className="text-3xl font-bold text-yellow-400">{stats?.successRate || 0}%</p>
        </div>
      </div>
      ...
      <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/10">
        <div className="flex flex-wrap gap-4">
          <div className="flex-1 min-w-[200px]">
            <div className="relative">
              ...
              <input
                ...
                className="w-full h-10 pl-10 pr-4 bg-white/10 border border-white/20 rounded-xl text-white placeholder:text-white/50 focus:outline-none focus:ring-2 focus:ring-[#4fa3d1]"
              />
            </div>
          </div>
          <select
            ...
            className="h-10 px-4 bg-white/10 border border-white/20 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-[#4fa3d1]"
          >
          ...
          <button
            onClick={loadLogs}
            className="h-10 px-4 bg-[#4fa3d1] text-white rounded-xl flex items-center gap-2 hover:bg-[#3d8ab8] transition-colors"
          >
      ...
      {recentFailures.length > 0 && (
        <div className="bg-red-500/20 border border-red-500/30 rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-red-300 font-semibold">最近失败的登录尝试</span>
          </div>
          <div className="space-y-2">
            {recentFailures.slice(0, 5).map((f, i) => (
              <div key={i} className="text-sm text-red-200/80">
                <span className="font-mono">{f.email}</span> - IP: {f.ip_address} - {new Date(f.created_at).toLocaleString('zh-CN')}
              </div>
            ))}
          </div>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/10">
                <th className="text-left p-4 text-white/70 text-sm font-medium">时间</th>
                <th className="text-left p-4 text-white/70 text-sm font-medium">邮箱</th>
                <th className="text-left p-4 text-white/70 text-sm font-medium">类型</th>
                <th className="text-left p-4 text-white/70 text-sm font-medium">状态</th>
                <th className="text-left p-4 text-white/70 text-sm font-medium">IP地址</th>
                <th className="text-left p-4 text-white/70 text-sm font-medium">错误信息</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                ...
                logs.map((log) => (
                  <tr key={log.id} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    <td className="p-4 text-white/80 text-sm font-mono">
                      {log.email}
                    </td>
                    <td className="p-4 text-white/80 text-sm">
...
// 用户行为追踪面板组件
function AnalyticsTrackingPanel({ token }: { token: string }) {
  const [events, setEvents] = useState<any[]>([]);
  const [stats, setStats] = useState<{
    total: number;
    eventTypeCount: Record<string, number>;
    eventNameCount: Record<string, number>;
    deviceTypeCount: Record<string, number>;
  } | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  ...
  const loadEvents = async () => {
    ...
    try {
      ...
      if (data.total !== undefined) {
        setStats({
          total: data.total,
          eventTypeCount: data.eventTypeCount || {},
          eventNameCount: data.eventNameCount || {},
          deviceTypeCount: data.deviceTypeCount || {},
        });
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-white">用户行为追踪</h2>
        <button
          onClick={loadEvents}
          className="h-10 px-4 bg-[#4fa3d1] text-white rounded-xl flex items-center gap-2 hover:bg-[#3d8ab8] transition-colors"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-white/70 text-sm">总事件数</span>
          </div>
          <p className="text-3xl font-bold text-white">{stats?.total || 0}</p>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-white/70 text-sm">页面浏览</span>
          </div>
          <p className="text-3xl font-bold text-green-400">
            {stats?.eventTypeCount?.page_view || 0}
          </p>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-white/70 text-sm">生成次数</span>
          </div>
          <p className="text-3xl font-bold text-yellow-400">
            {stats?.eventNameCount?.video_prompt_generate || 0}
          </p>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-white/70 text-sm">错误数</span>
          </div>
          <p className="text-3xl font-bold text-red-400">
            {stats?.eventTypeCount?.error || 0}
          </p>
        </div>
      </div>

      {/* 设备分布 */}
      {stats?.deviceTypeCount && Object.keys(stats.deviceTypeCount).length > 0 && (
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 border border-white/10">
          <h3 className="text-lg font-semibold text-white mb-4">设备类型分布</h3>
          <div className="flex gap-4">
            {Object.entries(stats.deviceTypeCount).map(([device, count]) => (
        ...
        <div className="overflow-x-auto max-h-[500px]">
          <table className="w-full">
            <thead className="sticky top-0 bg-[#1a3a6b]">
              <tr className="border-b border-white/10">
                <th className="text-left p-4 text-white/70 text-sm font-medium">时间</th>
                <th className="text-left p-4 text-white/70 text-sm font-medium">事件类型</th>
                <th className="text-left p-4 text-white/70 text-sm font-medium">事件名称</th>
                <th className="text-left p-4 text-white/70 text-sm font-medium">设备</th>
                <th className="text-left p-4 text-white/70 text-sm font-medium">详情</th>
              </tr>
            </thead>
            <tbody>
              {isLoading ? (
                ...
                events.map((event, i) => (
                  <tr key={event.id || i} className="border-b border-white/5 hover:bg-white/5">
                    <td className="p-4 text-white/80 text-sm">
                    ...
                    <td className="p-4 text-white/60 text-sm max-w-[200px] truncate">
                      {event.event_data ? (
                        <span className="font-mono text-xs">{event.event_data.slice(0, 50)}...</span>
                      ) : '-'}
                    </td>
                  </tr>
                ))
...
function AdminDashboard({ token, user, onLogout }: { token: string; user: AdminUser; onLogout: () => void }) {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'analytics' | 'logs' | 'tracking' | 'invites' | 'redemption' | 'users' | 'health'>('analytics');
  ...
  const menuItems = [
    ...
    { id: 'logs', icon: Shield, label: '登录日志' },
    { id: 'tracking', icon: Activity, label: '用户行为' },
    { id: 'invites', icon: Gift, label: '邀请码管理' },
    { id: 'redemption', icon: Ticket, label: '兑换码管理' },
    { id: 'users', icon: Users, label: '用户管理' },
    { id: 'health', icon: Database, label: '系统健康' },
  ] as const;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0a192f] via-[#1a3a6b] to-[#0a192f]">
      ...
      <div className="lg:ml-64 p-4 lg:p-8 pt-16 lg:pt-8">
        ...
        {/* 登录日志 */}
        {activeTab === 'logs' && (
          <LoginLogsPanel token={token} />
        )}

        {/* 用户行为追踪 */}
        {activeTab === 'tracking' && (
          <AnalyticsTrackingPanel token={token} />
        )}
